...
namespace Challenge
{
	...
	public class WordsStatistics_Tests
	{
		...
		public void GetStatistics_ContainsManyItems_AfterAdditionOfDifferentWords()
		{
			...
			Console.WriteLine(wordsStatistics.GetStatistics()); // очень странно

			wordsStatistics.GetStatistics().Should().HaveCount(2);
		}

		[Test, Timeout(10)]
		public void GetStatistics_withManyWords() {
			wordsStatistics.AddWord("abc");
			wordsStatistics.AddWord("def");
			wordsStatistics.GetStatistics().Should().HaveCount(2);
		...
		// Документация по FluentAssertions с примерами : ooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo
	}
}